...
same (symbol, strategy) on every run, leaving several identical open calls. This
command enforces the new invariant retroactively: for each (symbol, service)
pair it keeps the most recent PENDING signal and deletes the older duplicates.
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        for sig in pending:
            groups[(sig.symbol_id, sig.service_id)].append(sig)

        dupe_ids = []
        for (sym_id, svc_id), sigs in groups.items():
            if len(sigs) <= 1:
                continue
            keeper, *dupes = sigs  # keeper is the newest
            label = f"{keeper.symbol.ticker} / {keeper.service.name}"
            self.stdout.write(
